...
#define DISPATCH_ID_TO_VIRTUAL_LINEAR(blockListSRV, params) \
	int3 DispatchIDToVirtual(uint3 tidx, out int3 vBlockIdx, out uint3 cellIdx) \
	{ \
		uint blockID = tidx.y; \
		cellIdx = int3( \
		...
		{ \
			...
			vidx = (vBlockIdx << params.blockDimBits.xyz) + (cellIdx - params.linearBlockOffset.xyz); \
		} \
		return vidx; \
	}
...
#define DISPATCH_ID_TO_VIRTUAL2(blockListSRV, params) \
	...
	{ \
		...
		int3 vBlockIdx = tableVal_to_coord(blockListSRV[blockID]); \
		int3 vidx = (vBlockIdx << params.blockDimBits.xyz) | ((tidx << 1) & (params.blockDim.xyz - int3(1,1,1))); \
		return vidx; \
	}
