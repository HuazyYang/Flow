...
groupshared float2 sdata[THREAD_DIM_Z][THREAD_DIM_Y][3 * THREAD_DIM_X];
...
void execute(uint3 tidx, uint3 threadIdx)
{
	...
	sdata_write(threadIdx + int3(0 * THREAD_DIM_X, 0, 0), fieldSRV[VirtualToReal(vidx + int3(-THREAD_DIM_X, 0, 0))]);
	sdata_write(threadIdx + int3(1 * THREAD_DIM_X, 0, 0), fieldSRV[ridx]);
	sdata_write(threadIdx + int3(2 * THREAD_DIM_X, 0, 0), fieldSRV[VirtualToReal(vidx + int3(+THREAD_DIM_X, 0, 0))]);
	...
	for (int offset = -KERNEL_HALF_DIM; offset <= KERNEL_HALF_DIM; offset++)
	{
		...
		sum += weight * sdata_read(threadIdx + int3(offset + THREAD_DIM_X, 0, 0));
	}
...
void particleSurfaceSmoothXCS(uint3 tidx : SV_DispatchThreadID, uint3 blockIdx : SV_GroupID, uint3 threadIdx : SV_GroupThreadID)
{
