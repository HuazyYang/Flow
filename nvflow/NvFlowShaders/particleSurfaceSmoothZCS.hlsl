...
groupshared float2 sdata[3 * THREAD_DIM_Z][THREAD_DIM_Y][THREAD_DIM_X];
...
void execute(uint3 tidx, uint3 threadIdx)
{
	...
	sdata_write(threadIdx + int3(0, 0, 0 * THREAD_DIM_Z), fieldSRV[VirtualToReal(vidx + int3(0, 0, -THREAD_DIM_Z))]);
	sdata_write(threadIdx + int3(0, 0, 1 * THREAD_DIM_Z), fieldSRV[ridx]);
	sdata_write(threadIdx + int3(0, 0, 2 * THREAD_DIM_Z), fieldSRV[VirtualToReal(vidx + int3(0, 0, +THREAD_DIM_Z))]);
	...
	for (int offset = -KERNEL_HALF_DIM; offset <= KERNEL_HALF_DIM; offset++)
	{
		...
		sum += weight * sdata_read(threadIdx + int3(0, 0, offset + THREAD_DIM_Z));
	}
...
void particleSurfaceSmoothZCS(uint3 tidx : SV_DispatchThreadID, uint3 blockIdx : SV_GroupID, uint3 threadIdx : SV_GroupThreadID)
{
