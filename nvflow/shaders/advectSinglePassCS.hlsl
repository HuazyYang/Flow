...
Texture3D<float4> densitySRV : register(t6);
Texture3D<uint> densityBlockTable : register(t7);

//! A fade field for softer boundary conditions
Texture3D<float> fadeFieldSRV : register(t8);

//! A coarse density representation
Texture3D<float4> coarseDensitySRV : register(t9);
RWTexture3D<float4> coarseDensityUAV : register(u1);

//! Emitter parameters
Buffer<float4> emitterParametersSRV : register(t10);
//! SDF field, used in emitterFunctions.hlsli
Texture3D<float> sdf_SRV : register(t11);
...
void advectSinglePassCS(uint3 tidx : SV_DispatchThreadID, uint3 threadIdx : SV_GroupThreadID)
{
	int3 vidx;
	int3 ridx;
	...
	// compute semi-lagrangian result
	float4 vel = sampleVelocity(compute_vidxNormOld(vidxNorm));
	float3 fetchIdxf = compute_vidxOldf(vidxf) - deltaTime.xyz * vel.xyz;
	float4 predict = sampleValue(fetchIdxf);
	...
	if (shouldCorrect)
	{
		...
		float4 predictRev = 0.f.xxxx;
		{
			float3 sample_frac = frac(predictFetchIdxf - 0.5f.xxx);
			float3 sample_fracInv = 1.f.xxx - sample_frac;
			// need compute semi-Lagrangian result for each sample
			for (uint sampleIdx = 0u; sampleIdx < 8u; sampleIdx++)
			{
				int3 sample_vidx = int3(floor(predictFetchIdxf - 0.5f.xxx)) + int3(sampleIdx & 1, (sampleIdx >> 1) & 1, (sampleIdx >> 2) & 1);
				float3 sample_vidxf = float3(sample_vidx)+0.5f.xxx;
				float3 sample_vidxNorm = valueParams.vdimInv.xyz * sample_vidxf;
				float4 sample_vel = sampleVelocity(compute_vidxNormOld(sample_vidxNorm));
				float3 sample_fetchIdxf = compute_vidxOldf(sample_vidxf) - deltaTime.xyz * sample_vel.xyz;
				float4 sample_value = sampleValue(sample_fetchIdxf);

				float w = ((sampleIdx & 1) ? sample_frac.x : sample_fracInv.x) *
					(((sampleIdx >> 1) & 1) ? sample_frac.y : sample_fracInv.y) *
					(((sampleIdx >> 2) & 1) ? sample_frac.z : sample_fracInv.z);

				predictRev += w * sample_value;
			}
		}
